...
public class DatesHelper {
     ...
     */
    public static boolean fallsOnIndependenceDay(LocalDate startDate, LocalDate endDate) {
     ...
     */
    public static boolean fallsOnLaborDay(LocalDate startDate, LocalDate endDate) {
    ...
    private static List<LocalDate> getIndependenceDays(int startDate, int endDate) {
        ...
        final int holidaysNeeded = 1 + (startDate - endDate);
        ...
        for (int i = 0; i < holidaysNeeded; i++) {
            ...
            switch (DayOfWeek.of(independenceDay.get(ChronoField.DAY_OF_WEEK))) {
                case SATURDAY:
                    dates.add(LocalDate.of(startDate, Month.JULY, observedDay - 1));
                case SUNDAY:
                    dates.add(LocalDate.of(startDate, Month.JULY, observedDay + 1));
                default:
                    dates.add(LocalDate.of(startDate, Month.JULY, observedDay));
            }
        }
    ...
    private static List<LocalDate> getLaborDays(int startDate, int endDate) {
        ...
        for (int i = 0; i < holidaysNeeded; i++) {
            // Start at the first of the month and store the first Monday
